	...
	set(name, value) {
		...
		switch(name) {
			...
			case "commands": {
				...
				const createCategory = (folder, categoryName = folder, use = true) => {
					const category = module.exports.Category(categoryName, Object.assign({}, this.get("commandOptions") || {})); // eslint-disable-line new-cap, max-len
					const files = fs.readdirSync(path.resolve(value, folder));

					const aliases = files.find(file => ~[".js", ".json"].indexOf(path.extname(file)) &&
						path.basename(file, path.extname(file)) === "aliases");
					if(aliases) {
						category.aliases = require(path.resolve(value, folder, aliases))
					...
					files.filter(file => path.extname(file) === ".js")
						...
						.forEach(file => {
							...
							const run = scriptExports.run;
							delete scriptExports.run;

							category.command(path.basename(file, ".js"), scriptExports, run);
						});

					files.filter(file => fs.lstatSync(path.resolve(value, folder, file)).isDirectory())
						.forEach(subFolder => {
							const subcommands = createCategory(path.join(folder, subFolder), subFolder, false);
							category.use(subcommands);
						});

					if(use) this.use(category);
					return category;
				};

				fs.readdirSync(value)
					.filter(file => fs.lstatSync(path.resolve(value, file)).isDirectory())
					.forEach(folder => createCategory(folder));
	...
	_listen(eventName) {
		...
		const handler = async (...params) => {
			let allMiddles = this._middleware.reduce((total, mid) => {
				if(mid.type === "category" && !mid._function) {
					total = total.concat(
						mid.category._middleware
							.map(mid2 => mid2.type === "subcommand" ? mid2.subCommands._middleware : mid2)
