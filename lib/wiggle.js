...
class Wiggle {
	constructor() {
		...
		this.use("message", async (message, next, wiggle) => {
			if(this._options.localeFunction) await this._options.localeFunction(message);
		...
		switch(name) {
			...
			case "commands": {
				...
				fs.readdirSync(value)
					...
					.forEach(folder => {
						const category = module.exports.Category(folder, Object.assign({}, this._options.commandOptions)); // eslint-disable-line new-cap, max-len
