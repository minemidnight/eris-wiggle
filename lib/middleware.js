...
module.exports = (wiggle, ...callback) => {
	const middleware = { type: "global", priority: 1 };
	if(wiggle instanceof Category) middleware.category = wiggle.name;

	if(typeof callback[0] !== "function" || (typeof callback[0] === "object" && !callback[0].middleware)) {
		...
		} else if(callback[0] instanceof Category && wiggle instanceof Category) {
			middleware.type = "subcommand";
			middleware.subCommands = callback[0];
			middleware.name = callback[0].name;
	...
	if(typeof callback[0] === "object" && callback[0].middleware) {
		middleware._function = callback[0].middleware;
		middleware.priority = callback[0].priority;
	} else {
