...
const events = {
	...
	typeStarted: "typingStart",
	unavailableGuildCreate: "unavailableGuildCreate",
	unavailableGuildAdd: "unavailableGuildCreate",
	badGuildCreate: "unavailableGuildCreate",
	badGuildAdd: "unavailableGuildCreate",
	unknown: "unknown",
	NA: "unknown",
...
module.exports = (wiggle, ...callback) => {
	...
	if(typeof callback[0] !== "function") {
		if(events[callback[0]]) {
			...
			middleware.category = callback[0];
			return middleware;
		} else if(wiggle._middleware && wiggle._middleware.find(mid => mid.name === callback[0])) {
			const target = wiggle._middleware.find(mid => mid.name === callback[0]);
			return module.exports(wiggle, target.command, callback[1]);
		} else {
			middleware.type = "unknown";
			middleware.name = callback[0];
		}
